...
// con esto conseguimos sacar la profundidad de los puntos (sin hacer uso de la medida en pixeles)
bool encuentraProfundidad(
    rs2::depth_frame const & frame,
    float const depth_scale,
    dlib::full_object_detection const & face,
    markup_68 markup_from, markup_68 markup_to,
...
{   
    ...
    {
        auto pt = face.part( i );
        auto depth = frame.get_distance(pt.x(),pt.y());
        if( !depth )
            continue;
        // restamos la medida desde la nariz hasta el punto pasado para calcular la profundidad
        average_depth += (depth * depth_scale) - profundidad;  
        ++n_points;
    }
    if( !n_points )
        return false;
    *p_average_depth = average_depth;
    return true;
}
...
)
{
    ...
    if( !find_depth_from( frame, depth_scale, face, markup_68::NOSE_RIDGE_2, markup_68::NOSE_TIP, &nose_depth ) )
        return false;
    //*deepNose = nose_depth; // Guardamos el valor de la profundidad de la nariz
    ...
    // Prueba de calculo de distancia de un punto
    float profundidad;
    encuentraProfundidad(frame, depth_scale, face, markup_68::NOSE_TIP, markup_68::NOSE_TIP, &profundidad,0.0);

    // Calculamos la profundidad que hay desde la punta de la nariz hasta la boca
    float distBoca;
    encuentraProfundidad(frame, depth_scale, face, markup_68::MOUTH_OUTER_R, markup_68::MOUTH_INNER_7, &distBoca, profundidad);
    *deepMouth = distBoca;

    // Calculamos la profundidad de los ojos (suma de la profundidad con los dos ojos)
    float distOjos;
    encuentraProfundidad(frame, depth_scale, face, markup_68::RIGHT_EYE_FROM, markup_68::LEFT_EYE_TO, &distOjos, profundidad);
    *deepEye = distOjos;    
